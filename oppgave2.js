...
// For å kunne filtrere etter navn.
let nameField = document.getElementById('name');
nameField.addEventListener('change', () => {
  let searchText = nameField.value;
  let userHit = (users.find(users => users.name === searchText));
  // La oss gi brukeren en sjanse til å rykke tilbake til start med try catch.
  try {
  userUl.innerHTML = `<li><span>${userHit.id}</span><span>${userHit.name}</span><span>${userHit.age}</span></li>`;
}
  catch (err) {
    userUl.innerHTML = "";
    createTableUI(users);
...
//For å kunne filtrere på alder:
let ageField = document.getElementById('age');
let ageFilter = document.getElementById('filter');

ageFilter.addEventListener('click', () => {
  ageField = ageField.value;
  console.log(ageField)
  let ageHit = (users.find(users => users.age == ageField));
  try{
  userUl.innerHTML = `<li><span>${ageHit.id}</span><span>${ageHit.name}</span><span>${ageHit.age}</span></li>`;
  }
  catch(err) {
  userUl.innerHTML = "";
  createTableUI(users);
